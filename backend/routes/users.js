...
// 获取所有用户列表（仅管理员可访问）
router.get('/users', authenticateToken, checkRole(['admin', 'resource_coordinator', 'tech_manager']), (req, res) => {
  // 支持分页和搜索
  ...
  User.db.query(countQuery, params, (err, countResult) => {
    if (err) {
    ...
    User.db.query(query, queryParams, (err, results) => {
      if (err) {
...
// 获取单个用户详情
router.get('/users/:id', authenticateToken, checkRole(['admin', 'resource_coordinator', 'tech_manager']), (req, res) => {
  const userId = req.params.id;

  User.getUserById(userId, (err, user) => {
    if (err) {
      logger.error(`查询用户详情失败: ${err.message}`);
      return res.status(500).json({
        success: false,
        message: '获取用户信息失败',
        error: err.message
...
router.put('/users/:id', authenticateToken, checkRole(['admin']), (req, res) => {
  ...
  const validRoles = ['resource_coordinator', 'tech_manager', 'consultant', 'user', 'admin'];
